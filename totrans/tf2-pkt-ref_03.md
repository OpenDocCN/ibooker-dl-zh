...
[图3-1](#titanic_dataset_as_a_pandas_dataframe)展示了`titanic_df`作为pandas DataFrame的样子。
...
减少分类列维度的另一种方法是使用*哈希特征列*。该方法根据输入数据计算哈希值，然后为数据指定一个哈希桶。以下代码将“class”列的维度减少到4：
