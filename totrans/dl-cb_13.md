...
变分自动编码器背后的关键思想是在损失函数中添加一个项，表示图像和抽象表示之间的统计分布差异。为此，我们将使用Kullback-Leibler散度。我们可以将其视为概率分布空间的距离度量，尽管从技术上讲它不是距离度量。[维基百科文章](http://bit.ly/k-l-d)中有详细信息，供那些想要了解数学知识的人阅读。
