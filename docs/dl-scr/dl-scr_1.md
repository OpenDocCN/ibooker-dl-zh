...
² 在整个过程中，我将提供指向 GitHub 存储库的相关补充材料的链接，该存储库包含本书的代码，包括[本章](https://oreil.ly/2ZUwKOZ)的代码。

³ 在接下来的部分中，我们将专注于计算`N`相对于`X`的梯度，但相对于`W`的梯度也可以通过类似的方式推理。

⁴ 我们在“矩阵链规则”中进行了这样的操作。

⁵ 完整的函数可以在[书的网站](https://oreil.ly/deep-learning-github)找到；它只是前一页显示的`matrix function backward sum`函数的一个子集。
