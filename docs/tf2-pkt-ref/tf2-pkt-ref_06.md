...
交叉验证和测试数据集各包含一个 500 样本批次。代码设置参数以通知训练过程应该期望多少批次的训练和验证数据。训练数据的参数是`STEPS_PER_EPOCH`。交叉验证数据的参数是`VALIDATION_STEPS`，设置为 1，因为数据大小和批次大小都是 500。请注意，双斜杠（//）表示*地板除法*（即向下取整到最接近的整数）。
